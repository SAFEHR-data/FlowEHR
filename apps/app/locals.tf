...
locals {
  ...
  branches_and_envs = var.app_config.add_testing_slot ? {
    string(local.core_branch_name)    = local.core_gh_env
    string(local.testing_branch_name) = local.testing_gh_env
  } : { string(local.core_branch_name) = local.core_gh_env }
  ...
  envs_and_workflow_templates = var.app_config.add_testing_slot ? {
    string(local.core_gh_env)    = data.template_file.core_github_workflow
    string(local.testing_gh_env) = data.template_file.testing_github_workflow[0]
  } : { string(local.core_gh_env) = data.template_file.core_github_workflow }

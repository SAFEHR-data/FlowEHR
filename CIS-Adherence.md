...
| | `CIS 3.3`: Enable key rotation reminders for each storage account | n/a | Storage keys are not used for authentication |
| | `CIS 3.4`: Ensure that Storage Account Access keys are periodically regenerated | n/a | Storage keys are not used for authentication |
...
| | `CIS 10.1`: Ensure that resource locks are set for critical resources | n/a | To be performed manually, post-deployment. https://github.com/UCLH-Foundry/FlowEHR/issues/124 |
...
| | `CIS 4.2.1`: Ensure Microsoft Defender for SQL is set to 'on' | n/a | Set at org level |
